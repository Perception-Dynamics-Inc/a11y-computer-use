...
``a11y_computer_use bench web`` measures what one observation costs. This module
measures what a whole task costs, and whether it gets done: the same planner
model runs every task in the suite under `a11y_computer_use/arena_tasks/` through

* ``refs``         the reference agent loop (`a11y_computer_use.agent.run_task`):
...
* ``pixels+snap``  the same screenshot-only loop, but coordinate clicks that land
                   ...
                   (the hybrid a pixel client gets from `a11y_computer_use.adapters`).
...
from a11y_computer_use import agent, safety, server
from a11y_computer_use.adapters.anthropic_computer import AnthropicComputerAdapter
from a11y_computer_use.agent import DONE_TOOL, AgentResult, Step
from a11y_computer_use.providers import PlannerTurn, Provider, ProviderError, Usage
...
class Harness:
    ...
    Args:
        ...
        workdir: Where the permission store and audit log for the runs live
            ...
            the user's own ``~/.a11y_computer_use``.
