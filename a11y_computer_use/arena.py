...
* `run_desktop_task` measures whatever app the OS driver (or any driver) shows,
  ...
  re-observe diff in each view (``a11y_computer_use bench desktop``), so the desktop
...
class Report:
    ...
    def to_dict(self) -> dict:
        """JSON-ready form (``a11y_computer_use bench web --json``)."""
        return {
            "mode": self.mode,
...
class DesktopReport:
    ...
    def to_dict(self) -> dict:
        """JSON-ready form (``a11y_computer_use bench desktop --json``)."""
        return {
            "app": self.app,
            "rounds": self.rounds,
