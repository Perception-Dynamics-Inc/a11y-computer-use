...
18 tools: `desktop_snapshot`, `find`, `screenshot`, `zoom`, `click`, `type`, `key`, `scroll`, `drag`, `wait_for`, `act`, `set_value`, `scroll_to_find`, `app`, `window`, `clipboard`, plus `console` and `network` on the browser. Details: [docs/agent-loop.md](./docs/agent-loop.md) and the tool docstrings.
