# Contributing to a11y-computer-use

a11y-computer-use is an accessibility-first computer-use framework: pruned accessibility-tree snapshots, element refs, and a safety layer, exposed as an MCP server and a CLI (paraphrasing the `description` in `pyproject.toml`). This guide covers the development setup on each platform, how the test suite and its platform gates work, what CI runs, the rule that keeps the core platform-free, how to add an MCP tool, and the conventions for commits and pull requests.

The GitHub repository is private at the time of writing. The default branch is `main`; recent work has landed on `main`, and CI runs on pushes to both branches and on every pull request (`.github/workflows/ci.yml`). Read `docs/decisions/plan-2026-07.md` before proposing a scope change: the README's Contributing section points to it because it holds the positioning and the explicit anti-goals (section 4) and records the original architecture. Its status note at the top marks it a dated design record (front matter 2026-07-02) that has not been revised for the Windows observe/act loop or the Linux and browser backends, so take the current state from `README.md`, `CHANGELOG.md`, and the per-backend docs under `docs/`, not from `docs/decisions/plan-2026-07.md`.
...
```bash
git clone <repo-url> a11y-computer-use && cd a11y-computer-use
...
|---|---|---|---|
| macOS | `pip install -e ".[dev]"` (the pyobjc frameworks are core dependencies gated on `sys_platform == 'darwin'`) | none | the Accessibility TCC grant; Screen Recording as well for `screenshot` and `zoom`. `a11y-computer-use doctor` reports both. |
...
`a11y-computer-use doctor` runs six checks. The first names the app that owns the process tree, because TCC grants attach to that responsible app (your terminal or IDE) and take effect after it is relaunched:
...
Each `[FAIL]` line is followed by a `fix:` line naming the responsible app and a System Settings deep link (`render_text` in `a11y_computer_use/doctor.py`). `doctor` exits 0 even when checks fail.
...
The browser backend is never selected by platform; it is selected by name through `A11Y_COMPUTER_USE_DRIVER=browser` or `get_driver("browser")`. Start a Chromium that exposes DevTools, then point the server at it:
...
`A11Y_COMPUTER_USE_CDP_ENDPOINT` defaults to `http://127.0.0.1:9222` (`_DEFAULT_ENDPOINT` in `a11y_computer_use/drivers/browser.py`). The driver connects lazily, so `build_server()` succeeds without a browser; `tests/test_browser.py::test_mcp_server_exposes_console_and_network_only_on_browser` builds a server on `BrowserDriver(endpoint="x")` and lists its tools. In a container the CI job also passes `--no-sandbox --disable-gpu --disable-dev-shm-usage --no-first-run --no-default-browser-check` and polls `http://127.0.0.1:9222/json/version` until Chrome answers.

The MCP subcommand is `a11y-computer-use mcp`. `a11y-computer-use serve`, which still appears in the `get_driver` docstring (`a11y_computer_use/drivers/__init__.py:35`), is not a subcommand; `a11y-computer-use --help` lists `mcp`, `doctor`, `snapshot`, `run-once`, `bench`, and `agent` (the reference agent loop, see `docs/agent-loop.md`); `a11y-computer-use bench --help` lists the `audit`, `web`, `desktop`, and `h2h` sub-subcommands (`a11y_computer_use/cli.py`).
...
```bash
python -c "from a11y_computer_use.drivers import get_driver, current_platform; d = get_driver(); print('platform:', current_platform(), '| driver:', d.name)"
python -c "import a11y_computer_use.server as s; print(s.build_server().name)"   # prints: a11y-computer-use
a11y-computer-use doctor                                                          # macOS
```
...
|---|---|---|
| `A11Y_COMPUTER_USE_DRIVER` | selects the backend by name: `macos`, `windows`, `linux`, `browser`; default is the current OS. Any other value raises `NotImplementedError`. | `a11y_computer_use/drivers/__init__.py`, at each `get_driver()` call |
| `A11Y_COMPUTER_USE_CDP_ENDPOINT` | DevTools HTTP endpoint for the browser backend; default `http://127.0.0.1:9222` | `a11y_computer_use/drivers/browser.py` |
| `A11Y_COMPUTER_USE_AX_CLICKS` | `0` forces synthetic mouse clicks instead of an AX press for ref clicks | `a11y_computer_use/server.py`, once at import (`PREFER_AX_ACTIONS`) |
| `A11Y_COMPUTER_USE_CONFIRM` | `0` disables the destructive-click confirmation gate | `a11y_computer_use/server.py`, once at import (`CONFIRMATION_GATE`) |
| `A11Y_COMPUTER_USE_NO_WEB_A11Y` | any value disables the Chromium/Electron accessibility force-enable (macOS: per-app AX attributes; Linux: the session-wide `org.a11y.Status` flip) | `a11y_computer_use/observe.py` (macOS) and `a11y_computer_use/drivers/_atspi.py` (Linux) |
| `A11Y_COMPUTER_USE_ATSPI_EVENTS` | any value routes libatspi calls through a dedicated event thread; Linux only | `a11y_computer_use/drivers/_atspi_events.py` |
| `A11Y_COMPUTER_USE_SCREEN` | `WxH` fallback for the primary screen size when Xlib is unavailable; default `1280x800`; Linux only | `a11y_computer_use/drivers/_atspi.py` |
| `A11Y_COMPUTER_USE_OVERLAY_RGBA` | macOS only. `r,g,b` or `r,g,b,a` color for the demo overlay, 0..1 floats or 0..255 ints; default `0.16,0.55,1.0,1.0` | `a11y_computer_use/overlay.py` (`_resolve_rgba`) |
| `A11Y_COMPUTER_USE_PROVIDER` | planner backend for `a11y-computer-use agent` and `a11y-computer-use bench h2h`: `anthropic`, `openai`, `claude-cli` (and `scripted`, Python only). Unset picks the first the environment supports: an Anthropic key, then an OpenAI key or base URL, then the `claude` command. Not read by the MCP server. | `a11y_computer_use/providers.py` (`get_provider`) and `a11y_computer_use/cli.py` (`_cmd_bench_h2h`) |
| `ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_BASE_URL`, `OPENAI_API_KEY` / `OPENAI_BASE_URL` | credentials and endpoint for the `anthropic` and `openai` planners of `a11y-computer-use agent` and `bench h2h`; also drive the provider auto-pick above. `OPENAI_BASE_URL` points at Ollama, vLLM, or OpenRouter; the key is required only for `api.openai.com`. `claude-cli` needs no key, only the local `claude` command. Not read by the MCP server. | `a11y_computer_use/providers.py` (`AnthropicProvider.__init__`, `OpenAIProvider.__init__`, `get_provider`) |

Because `A11Y_COMPUTER_USE_AX_CLICKS` and `A11Y_COMPUTER_USE_CONFIRM` are read when `a11y_computer_use.server` is imported, set them in the environment of the process that imports the module, not inside it.
...
To run the macOS live tests, grant Accessibility (and Screen Recording for capture) to the terminal or IDE that runs pytest, relaunch it, and confirm with `a11y-computer-use doctor`. `tests/e2e/test_mcp_stdio.py` spawns `python -m a11y_computer_use mcp` as a real subprocess with `HOME` pointed at a pytest temp dir, so it never reads or writes `~/.a11y-computer-use`.
...
The workflow triggers on `push` to `main` and `main` and on `pull_request`.

## The driver seam rule

```mermaid
flowchart TD
    host["MCP host or in-process embedder"] --> server["a11y_computer_use/server.py: build_server and Runtime, which gates, rechecks, and audits"]
    server --> safety["a11y_computer_use/safety.py: tiers, PermissionStore, AuditLog"]
    server --> schema["a11y_computer_use/schema.py: Element, Snapshot, Action, ErrorCode"]
    server --> driver["a11y_computer_use/drivers/base.py: the Driver protocol"]
...
The docstring of `a11y_computer_use/drivers/base.py` states the contract: the canonical `schema`, the tree-pruning core (`observe.build_snapshot`), the safety layer, and the MCP server surface are platform-free; everything OS-specific (walking the accessibility tree, synthesizing input, capturing pixels, enumerating windows) lives behind the `Driver` protocol. Adding a platform is "implement the protocol", never "touch the core". The rules that follow from that:

1. Platform code goes in `a11y_computer_use/drivers/`. A backend is `drivers/<name>.py` plus private helpers named `_<name>_*.py` or by API (`_uia.py`, `_win_input.py`, `_win_system.py`, `_atspi.py`, `_atspi_events.py`, `_linux_input.py`, `_linux_system.py`, `_cdp.py`, `_cdp_ax.py`).
...
4. Register the name in `get_driver()` in `a11y_computer_use/drivers/__init__.py`, which resolves `name or $A11Y_COMPUTER_USE_DRIVER or current_platform()`. A backend that is not an OS (the browser) is selected only explicitly.
...
8. If your backend's "apps" are not OS processes, set the class attribute `resolves_apps = True`; the `Runtime` then resolves identity, frontmost checks, and the act-time recheck through the driver instead of the OS system-ops (comment above `Runtime._resolves_apps` in `a11y_computer_use/server.py`). The browser driver uses this so tabs act as apps.
...
6. Optional. If the tool should be reachable from `a11y-computer-use run-once`, add its name to `Runtime.RUN_ONCE_TOOLS`; `Runtime.dispatch` only checks that allowlist before delegating to `Runtime.call_tool`. Only string-returning action tools belong there, and refs are unavailable in that one-shot path.
7. Docs. Update the tool table in `README.md` and section 8 of `docs/decisions/plan-2026-07.md`, and the backend doc under `docs/` if the tool is backend-specific.
...
- The README's Contributing section asks that issues and discussion start from `docs/decisions/plan-2026-07.md`. The repository is private at the time of writing, so open discussion happens once it is public.
