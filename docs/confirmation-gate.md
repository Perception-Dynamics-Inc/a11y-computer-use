...
**2. Fail-safe when there is no channel.** If the host/transport can't elicit (a CLI `run-once` one-shot, or an MCP client that doesn't advertise elicitation), the irreversible action is **blocked** with a structured `confirmation_declined` error — never fired unconfirmed. Escape hatch: `COMPUTERUSE_CONFIRM=0` disables the gate wholesale for automation that has accepted the risk.
...
1. **Classifier** — `safety.confirmation_prompt(action, app)` returns a prompt string when a `Click` targets an element whose label matches a conservative destructive-verb set (`delete`, `move to trash`, `empty trash`, `discard`, `erase`, `uninstall`, `permanently`, `wipe`, `don't save`), else `None`. Deliberately conservative: a miss just means "no extra prompt"; a false positive nags on a safe click. `send`/`remove`/`reset` are intentionally excluded.
...
3. **Transport bridge** — the MCP `click` tool builds a sync confirmer that bridges the worker thread back to the event loop (`anyio.from_thread.run`) to call `ctx.elicit`. Elicitation unsupported → the confirmer returns False → fail-safe block.
