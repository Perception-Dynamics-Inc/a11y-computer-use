...
- Visual presence overlay: a blue screen-edge glow and an agent cursor in a standalone `computeruse.overlay` module with a demo (`python -m computeruse.overlay`). macOS-only; not wired into the MCP server (0b8eab0, 2026-07-13).
...
- `BrowserDriver` over the Chrome DevTools Protocol: `Accessibility.getFullAXTree` and `DOMSnapshot` fused into the shared observe engine; coordinate-free press, focus, `Input.insertText`, and `Input.dispatchKeyEvent`. Selected only by `COMPUTERUSE_DRIVER=browser` or `get_driver("browser")`, never by platform. Adds the `[browser]` extra (websocket-client), `docs/browser-backend.md`, and the wider package description (58ec018, 2026-08-29).
...
- Linux: opt-in AT-SPI event thread (`COMPUTERUSE_ATSPI_EVENTS=1`) that trusts libatspi's read cache. The commit message reports about 1.8x on its own measurement; that figure is not reproduced in CI (3371405, 2026-08-29).
...
- COM-3: the name `computerUse` is kept (445f0b2, 2026-07-12).
