...
https://github.com/Perception-Dynamics-Inc/a11y-computer-use
...
- the tool call or Python call that triggered the behavior, and the grant state in `~/.a11y-computer-use/permissions.json` at the time
- what you expected the safety layer to do and what it did instead
- the relevant lines from `~/.a11y-computer-use/audit/<date>.jsonl`, with anything personal removed
...
The human operator is trusted. So is the host process that launches the server (`a11y-computer-use mcp`, which speaks MCP over stdio and opens no network port) or that imports `a11y_computer_use.server.Runtime` in-process. Anyone who can write to that process's stdin, edit its permission file, or set its environment variables has the same power as the operator. The safety layer does not defend the operator against local software running as the same user, and it is not a sandbox.
...
Every tool, observation included, goes through one method: `Runtime._run_gated` in `a11y_computer_use/server.py`. The order is fixed. Permission check, then confirmation, then a recheck of what is in front or under the pointer, then the driver call, then an audit entry. Refusals and driver errors are audited before they propagate.

Per-app permission tiers. `a11y_computer_use/safety.py` defines three tiers. `read` covers snapshots, `find`, screenshots, `zoom`, `wait_for`, clipboard reads, the browser `console` and `network` feeds, and the list verbs of `app` and `window`. `click` adds pointer actions plus the `launch`, `focus`, and `raise` verbs. `full` adds typed text, key chords, `set_value`, and clipboard writes. An app with no grant gets `needs_permission`, for reads as well as for input. An app on the deny list gets `deny` regardless of tier. A non-empty allow list denies everything not on it. Grants are keyed by app identity: bundle id on macOS, process image name on Windows, process comm name on Linux, CDP tab id on the browser backend. No tool or prompt lets the model grant a tier. The only CLI path that writes a grant is the operator-invoked `a11y-computer-use agent --grant`, which runs before the model sees anything. Grants are written by a human editing `~/.a11y-computer-use/permissions.json`, by an operator passing `a11y-computer-use agent --grant <read|click|full>` on the command line (which calls `PermissionStore.set_tier` for the target app before the agent loop starts and persists to the same file), or by an embedder calling `PermissionStore.set_tier`. The store re-reads the file when its modification time or size changes, so a deny added mid-session takes effect on the next call.

Same-window recheck. `type` and `key` re-read the frontmost app immediately before injection and abort with `focus_changed` if it is not the gated app. Ref-based `click`, `scroll`, `drag`, and `set_value` hit-test the target point and abort with `focus_changed` if another app's window now owns it. On the browser backend both rechecks compare against the bound tab (`tests/test_safety_hardening.py` pins the tab-switch case).
...
3. Secure-field handling is now the same shape on every backend, with these remaining limits:
   ...
   |---|---|---|---|---|
   | Password field marked in the snapshot, value never emitted | yes (`AXSecureTextField`) | yes (UIA `IsPassword` on edit controls; unit-tested, not live-verified) | yes (AT-SPI role `password text`) | yes (`<input type="password">`) |
   | `press_element` and `set_value` refuse a secure element | yes | yes | yes | yes |
   | Pointer action on a secure element refused (ref, or a raw point inside one) | yes | ref path yes; coordinate input not implemented | yes | yes (live-verified 2026-09-02) |
   | `type` refused while a password field has focus | yes (two probes) | yes (UIA focused control; unit-tested only) | yes (AT-SPI focus walk; unit-tested only) | yes (live-verified 2026-09-02) |

   The coordinate check reads the latest snapshot, so a password field that appeared after the last observation is not known to it, and on Linux the focus walk stops after 400 nodes. Key chords are not probed on any backend: a single-character chord typed into a focused password field is not refused. Revealing a secure field with `scroll(into_view=true)` is allowed; it injects nothing.
...
5. Two refusals happen before the gate and are logged as failures with `decision: null` (see the audit log paragraph above): `set_value` on a secure field, and a ref that fails to resolve for any ref-targeted tool. Invalid arguments (a bad modifier, an unparseable chord, a missing target) are rejected before the gate and are not audited. `window raise` resolves the window's owner before gating; a window that does not exist is reported as `app_not_found` without an audit row.
...
8. UI text is untrusted model input. Element titles, values (up to 200 characters each), placeholders, page console output, and request URLs are handed to the model as text. a11y-computer-use does not filter that text for instructions. A window or page that says "click Delete" is a prompt-injection vector by construction; the tiers and the confirmation gate are the mitigation, with the limits listed above.
9. `A11Y_COMPUTER_USE_CONFIRM` and `A11Y_COMPUTER_USE_AX_CLICKS` are read once, when `a11y_computer_use.server` is imported. Whoever controls the process environment controls the gate.
10. Two side effects reach beyond the target app. On macOS, snapshotting a Chromium or Electron app sets `AXManualAccessibility` and `AXEnhancedUserInterface` on that app so it builds its accessibility tree. On Linux, the driver sets `org.a11y.Status.IsEnabled` and `ScreenReaderEnabled` to true on the session bus, which tells Chromium and Electron apps already running in the session to build their accessibility trees; the flag is session-wide, not per app. `A11Y_COMPUTER_USE_NO_WEB_A11Y=1` turns both off.

### Files a11y-computer-use writes

`~/.a11y-computer-use/permissions.json` holds the grants:
...
`~/.a11y-computer-use/audit/YYYY-MM-DD.jsonl` holds one entry per gated call with the fields `ts`, `app`, `action`, `params`, `decision`, `result`, and (on success) `metrics`. `params` carries the action's arguments. For a click that means the target element's role, title, path, and bounds, with its value redacted. For `type` it is the typed text in the clear, unless a secure field was hit. For `key` it is the chord. So the log contains UI text: window and button titles, and whatever the model typed into non-secure fields. Treat it as sensitive.

Both files are created with the process's default file permissions. a11y-computer-use sets no restrictive mode and encrypts nothing. Embedders can move both locations; see below.

### What the model receives

A snapshot carries, for each kept element, its role, title, value (clipped to 200 characters in the data and 48 in the rendering, omitted for secure fields), placeholder, state flags, and geometry. A screenshot is the whole display, downscaled. A clipboard read is the operator's current clipboard. On the browser backend, `console` returns the page's console messages and uncaught exceptions, and `network` returns request method, URL, and status or failure. Through `a11y-computer-use mcp`, all of this leaves the machine if the model runs remotely; a11y-computer-use has no view of where the MCP host sends it. Through `a11y-computer-use agent`, a11y-computer-use sends it itself; see the next section.
...
- `claude-cli`: runs the local `claude` binary via `subprocess` with `--strict-mcp-config`; no HTTP from a11y-computer-use. Under `a11y-computer-use agent` no image content is passed (screenshots reach it as text). In the pixel modes of `a11y-computer-use bench h2h` the newest screenshot is written to a temporary PNG and the CLI is allowed only its `Read` tool to open it. Where that CLI sends the prompt is governed by its own configuration.

`A11Y_COMPUTER_USE_PROVIDER` or `--provider` selects the planner; with neither set, the first of those credentials found in the environment wins, then a `claude` binary on `PATH`. a11y-computer-use reads these credentials from the environment, stores none of them, and adds no verification of the endpoints beyond the standard library's default TLS handling. The `agent_step` and `agent_run` audit rows it writes carry the tool name, the parameters, the result, and planner token counts, not credentials. `bench h2h` keeps its permission store and audit log under a temporary directory rather than `~/.a11y-computer-use`.

`--grant read|click|full` writes a tier for the target app to `~/.a11y-computer-use/permissions.json` before the loop starts, exactly as if the operator had edited the file; it is an operator flag, not something the model can invoke. Every action the planner chooses still goes through `Runtime._run_gated`, so the tiers, confirmation, and audit described above apply unchanged, and the MCP server path (`a11y-computer-use mcp`) still opens no network port and makes no outbound requests.

### Browser backend

With `A11Y_COMPUTER_USE_DRIVER=browser`, the driver fetches `/json` from `A11Y_COMPUTER_USE_CDP_ENDPOINT` (default `http://127.0.0.1:9222`) and opens a WebSocket to the tab it binds. a11y-computer-use adds no authentication on top of the Chrome DevTools Protocol, and Chromium's remote debugging port has none of its own, so any process that can reach that port has full control of the browser regardless of a11y-computer-use's grants. Keep the port on loopback, as the default is, and do not forward it. Grants on this backend are keyed by CDP target id.

## Guidance for embedders

Your application is the responsible process for OS permissions. On macOS, TCC attributes the Accessibility and Screen Recording grants to the nearest ancestor `.app` bundle in the process tree; `a11y-computer-use doctor` names it. Your users grant those permissions to your app once, and everything you embed, a11y-computer-use included, inherits them. In the operating system's eyes you are the accountable party.

Beyond that:

- Your product decides grants. `PermissionStore.set_tier` is the only grant API (the `a11y-computer-use agent --grant` CLI flag is a thin operator-invoked wrapper over it), and a11y-computer-use ships no prompt to ask the human. Build that prompt yourself and never expose `set_tier` to the model.
- Pass a `confirm` callback into `Runtime.click` and `Runtime.act_batch`, or accept that destructive clicks are blocked. Do not ship with `A11Y_COMPUTER_USE_CONFIRM=0` unless your product has taken over that responsibility in some other way.
...
- Set `A11Y_COMPUTER_USE_CONFIRM` and `A11Y_COMPUTER_USE_AX_CLICKS` before importing `a11y_computer_use.server`, not after.
- If your agent loop is reachable over a network, authenticate there. a11y-computer-use trusts its caller and has no authentication of its own.
- On Windows there is no OS permission prompt to fall back on; a11y-computer-use's tiers are the only gate. On Linux the same is true, and the accessibility bus side effect in item 10 above applies to the whole session.
...
- UI content (titles, values, console output) that changes how a11y-computer-use itself parses or gates, for example by forging a snapshot header or an element ref.
- A problem in how a11y-computer-use uses one of its dependencies: `mcp`, `pillow`, the `pyobjc` frameworks, `websocket-client`, `uiautomation`, `PyGObject`, or `python-xlib`.

## Out of scope

- Anything that requires write access to `~/.a11y-computer-use` or to the server's process environment.
...
|---|---|---|
| `~/.a11y-computer-use/permissions.json` | Per-app tiers, deny list, allow list | On every check; re-read when the file changes |
| `~/.a11y-computer-use/audit/` | Audit log directory | On every gated call |
| `A11Y_COMPUTER_USE_CONFIRM=0` | Disables the destructive-click confirmation gate | Once, at import of `a11y_computer_use.server` |
| `A11Y_COMPUTER_USE_AX_CLICKS=0` | Forces synthetic mouse clicks instead of accessibility press actions | Once, at import of `a11y_computer_use.server` |
| `A11Y_COMPUTER_USE_DRIVER` | Selects the backend (`macos`, `windows`, `linux`, `browser`) | When the driver is created |
| `A11Y_COMPUTER_USE_CDP_ENDPOINT` | DevTools endpoint for the browser backend | When the browser driver is created |
| `A11Y_COMPUTER_USE_NO_WEB_A11Y` | Disables the Chromium/Electron and Linux accessibility force-enable | When a snapshot or the Linux driver's trust check runs |
| `A11Y_COMPUTER_USE_PROVIDER`, `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, `OPENAI_API_KEY`, `OPENAI_BASE_URL` | Planner and endpoint for `a11y-computer-use agent` and `a11y-computer-use bench h2h`; not read by the MCP server | When the provider is constructed (`a11y_computer_use/providers.py`) |

The MCP transport is stdio only; `a11y-computer-use mcp` calls `build_server().run(transport="stdio")` and listens on no port. The outbound requests described under Reference agent loop are made only by `a11y-computer-use agent` and `a11y-computer-use bench h2h`, never by the server.
