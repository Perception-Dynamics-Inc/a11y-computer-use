"""Opt-in AT-SPI event thread (``COMPUTERUSE_ATSPI_EVENTS=1``). Linux-only.
...
def enabled() -> bool:
    """True when the event thread is opted in via ``COMPUTERUSE_ATSPI_EVENTS``."""
    return bool(os.environ.get("COMPUTERUSE_ATSPI_EVENTS"))
...
def _ensure() -> None:
    ...
    with _start_lock:
        if _thread is not None:
            return
        t = threading.Thread(target=_run, name="computeruse-atspi", daemon=True)
        t.start()
        _ready.wait(5)
        _thread = t
