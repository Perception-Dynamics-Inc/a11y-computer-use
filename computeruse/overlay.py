"""Visual presence overlays — make computerUse *visible* without hijacking the
user's pointer.
...
cosmetic overlays so a human can see, at a glance, that computerUse is working
and where:

* **screen-edge glow** — a click-through, translucent border around the active
  display while computerUse holds a session/action, like a screen-recording rim.
* **agent cursor** — computerUse's own arrow marker (distinct shape + color)
...
#: computerUse's signature colour (electric blue, ~#2A8CFF) as RGBA 0..1.
#: Override at runtime via COMPUTERUSE_OVERLAY_RGBA="r,g,b" (0..1 or 0..255).
...
def _resolve_rgba() -> tuple[float, float, float, float]:
    """`BRAND_RGBA`, unless COMPUTERUSE_OVERLAY_RGBA overrides it as "r,g,b"
    (or "r,g,b,a"), accepting either 0..1 floats or 0..255 ints."""
    raw = os.environ.get("COMPUTERUSE_OVERLAY_RGBA", "").strip()
    if not raw:
        return BRAND_RGBA
    try:
...
class _CursorView(NSView):
    """Draws computerUse's arrow pointer (distinct from the OS cursor)."""
...
class Overlay:
    ...
    def __init__(self, rgba: tuple[float, float, float, float] | None = None) -> None:
        ...
        # stealing focus from the app computerUse is driving.
...
def demo(seconds: float = 8.0) -> None:
    ...
    print(f"Overlay shown on {int(scr.size.width)}x{int(scr.size.height)} pt screen "
          ...
          f"(computerUse's signature colour), gliding smoothly between points.")
